...
כלי AI בשימוש: **ChatGPT בתוך Cursor Cloud**. הכלי שימש לתכנון, ניסוח האפיון, סקירת החלטות ארכיטקטורה, מימוש קוד, כתיבת בדיקות, פתרון קונפליקטים ועדכון התיעוד.

### שלב א' - תכנון ואפיון

בתחילת העבודה בוצע תכנון פתרון ונכתב קובץ אפיון מרכזי בשם `SPECIFICATION.md`.  
בעזרת ChatGPT/Cursor Cloud גובש מבנה ראשוני, ונקבע שקובץ האפיון יתעדכן באופן דינמי לאורך כל המשימה כדי לשמור מקור אמת מעודכן להחלטות, לשינויים ולשיקולי הארכיטקטורה.
...
לאחר מכן נעשה שימוש ב-AI כדי לשאול ולחדד שאלות ארכיטקטורה ודיזיין: מה יקרה אם נרצה להרחיב לנתוני עתק, לתמוך במספר משתמשים גדול, לשנות מקור נתונים, להוסיף cache, לשנות מחלקות או להחליף תלויות.

### שלב ג' - ירידה לקוד ודיוק החלטות

לאחר שהכיוון הארכיטקטוני היה ברור יותר, בוצעה קריאה בפועל של הקבצים והקוד.  
בשלב זה נעזרתי ב-AI כדי לקרוא קבצים, לזהות נקודות לא ברורות ולבצע התאמות בקוד: הסרת MediatR כאשר התברר שאינו נדרש לשלב read-only, פיצול קומפוננטות Frontend, הוספת ולידציות, טיפול שגיאות, cache קצר בצד לקוח ושיפור החיפוש החופשי.
...
במהלך ההרצה נעזרתי ב-AI לשאלות נוספות על חוויית משתמש, Virtual Scroll, cache, שגיאות, JSON פגום ותרחישי עומס.

### שלב ה' - אימות מול הדרישות

בסיום בוצעה חזרה על ההנחיות והדרישות כדי לוודא שהן בוצעו במלואן ובמדויק.  
בעזרת AI בוצעה בדיקה חוזרת מול הדרישות, קובץ `SPECIFICATION.md` עודכן לאורך הדרך כדי לשקף את ההחלטות הסופיות, וה-README נשאר נקודת כניסה קצרה להרצה, בדיקות ותיאור התהליך.
