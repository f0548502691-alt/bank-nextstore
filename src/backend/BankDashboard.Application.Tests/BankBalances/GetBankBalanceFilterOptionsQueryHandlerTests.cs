...
public sealed class GetBankBalanceFilterOptionsQueryHandlerTests
{
    [Fact]
    public async Task Handle_ReturnsDistinctSortedFilterValues()
    {
        var handler = new GetBankBalanceFilterOptionsQueryHandler(new InMemoryBankBalanceReadRepository(
        [
        ...
        var response = await handler.Handle(new GetBankBalanceFilterOptionsQuery(), CancellationToken.None);
